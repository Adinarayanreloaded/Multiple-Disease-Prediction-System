...
diabetes_loaded_model = pickle.load(
    open('C:\\Users\\Asus\\PycharmProjects\\MDPS\\saved models\\trained_model4.sav', 'rb'))

heart_loaded_model = pickle.load(open('C:\\Users\\Asus\\PycharmProjects\\MDPS\\saved models\\heart_savfile.sav', 'rb'))

breast_loaded_model = pickle.load(
    open('C:\\Users\\Asus\\PycharmProjects\\MDPS\\saved models\\breat_cancer_random_forest_algorithm.sav', 'rb'))
...
def main():
    ...
    if selected == 'Diabetes Prediction':
        ...
        if st.button("diabetes test result"):
            input_data = [Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction,
                          Age]
            diagnosis = diabetes_prediction(input_data)

        if diagnosis == "the person is non diabetic":
    ...
    if selected == 'Breast Cancer Prediction':
        ...
        if st.button("Breast cancer test result"):
            input_data = [Clump_Thickness, Cell_Size, Cell_Shape, Marginal_Adhesion, Single_Epithelial_Cell_Size,
                          Bare_Nuclei, Bland_Chromatin, Normal_Nucleoli, Mitoses]
            diagnosis = breast_prediction(input_data)

        if diagnosis == "Breast cancer : **Negative**":
            st.success(diagnosis)
        else:
            st.warning(diagnosis)
    ...
    if selected == 'Heart Disease Prediction':
        ...
        with col3:
            cpy = st.selectbox('Chest Pain types',
        ...
        with col1:
            restecgy = st.selectbox('Resting Electrocardiographic results', (
        ...
        with col2:
            slopey = st.selectbox('Slope of the peak exercise ST segment',
        ...
        if diagnosis == "heart disease : **negative**":
            st.success(diagnosis)
        else:
            st.warning(diagnosis)
